...
function getCodegenRunner(config) {
  ...
  var writerConfigs = (0, _defineProperty2["default"])({}, sourceWriterName, {
    writeFiles: getRelayFileWriter(config.src, languagePlugin, config.noFutureProofEnums, artifactDirectory, config.persistOutput, config.customScalars, persistQueryFunction, config.repersist, config.includePersistedQueryTextInDev),
...
function getRelayFileWriter(baseDir, languagePlugin, noFutureProofEnums, outputDir, persistedQueryPath, customScalars, persistFunction, repersist, includePersistedQueryTextInDev) {
    ...
    var _ref2 = _asyncToGenerator(function* (_ref) {
      ...
      var results = yield RelayFileWriter.writeAll({
        config: {
          ...
          repersist: repersist,
          includePersistedQueryTextInDev: includePersistedQueryTextInDev
        },
