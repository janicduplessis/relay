  ...
  return {
    leave: {
      Root: function Root(node) {
        ...
        return t.program((0, _toConsumableArray2["default"])(getFragmentImports(state)).concat((0, _toConsumableArray2["default"])(getEnumDefinitions(state)), (0, _toConsumableArray2["default"])(inputObjectTypes), [inputVariablesType, responseType, operationType]));
      },
      Fragment: function Fragment(node) {
        ...
        var keyTypeName = getKeyTypeName(node.name);
        var keyTypeDataProperty = readOnlyObjectTypeProperty('$data', t.genericTypeAnnotation(t.identifier("".concat(node.name, "$data"))));
        keyTypeDataProperty.optional = true;
        var keyTypeFragmentRefProperty = readOnlyObjectTypeProperty('$fragmentRefs', t.genericTypeAnnotation(t.identifier("".concat(node.name, "$ref"))));
        var keyType = t.objectTypeAnnotation([keyTypeDataProperty, keyTypeFragmentRefProperty]);
        ...
        return t.program((0, _toConsumableArray2["default"])(getFragmentImports(state)).concat((0, _toConsumableArray2["default"])(getEnumDefinitions(state)), [importTypes(importedTypes, 'relay-runtime'), refType, exportType(node.name, type), exportType(dataTypeName, dataType), exportType(keyTypeName, keyType)]));
      },
